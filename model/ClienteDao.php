...
class ClienteDao
{
    ...
    {
        $sql = "INSERT INTO cliente(nome, sobrenome, data_nasc, cnpj, segEmpresa, email, senha ) VALUES(?,?,?,?,?,?,?)";
        ...
        $cadastrar->bindValue(5, $cliente->getRamo());
        $cadastrar->bindValue(6, $cliente->getEmail());
        $cadastrar->bindValue(7, $cliente->getSenha());
        $cadastrar->execute();
    ...
    {
        $sql = "SELECT nome AS 'nome_cli', sobrenome AS 'sobrenome_cli', DATE_FORMAT(data_nasc, '%d/%m/%Y') AS 'nascimento_cli', cnpj AS 'cnpj_cli',segEmpresa as 'ramo_cli', email AS 'email_cli', plano AS 'plano_atual' FROM cliente WHERE cod_cli = ?";
    ...
    {
        $sql ="UPDATE cliente SET nome = ?, sobrenome = ?, data_nasc = ?, cnpj = ?,segEmpresa = ?, email = ?, senha = ? WHERE cod_cli = ? ";
        ...
        $alterar->bindValue(5, $cliente->getRamo());
        $alterar->bindValue(6, $cliente->getEmail());
        $alterar->bindValue(7, $cliente->getSenha());
        $alterar->bindValue(8, $cliente->getCod());
